...
/**
 * Submit an enquiry to the 'enquiries' table
 * @param {Object} enquiryData - The enquiry data with listing_id, name, email, message, timestamp
 * @returns {Promise<Object>} Result object with success status
 */
export async function submitEnquiry(enquiryData) {
  // Log to console if Supabase is not configured
  if (!isSupabaseConfigured()) {
    console.log('📧 Enquiry submitted (Supabase not configured):', {
      listing_id: enquiryData.listing_id,
      name: enquiryData.name,
      email: enquiryData.email,
      message: enquiryData.message,
      timestamp: enquiryData.timestamp
    })
    return { success: true, message: 'Enquiry logged to console' }
  }
  ...
  try {
    ...
    if (error) {
      ...
      console.log('📧 Enquiry submitted (fallback to console):', enquiryData)
      return { success: true, message: 'Enquiry logged to console' }
    }

    console.log('✅ Enquiry submitted successfully to Supabase')
    return { success: true, data, message: 'Enquiry submitted successfully' }
  } catch (error) {
